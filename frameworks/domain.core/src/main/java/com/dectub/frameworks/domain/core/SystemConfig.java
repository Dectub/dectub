package com.dectub.frameworks.domain.core;

/**
 * @author Created by Neil Wang
 * @version 1.0.0
 * @date 2021/9/23 3:02 下午
 */
public final class SystemConfig {
//    public static final String CREATED = "created";
    public static final String REGISTER_EMAIL = "register.email";
    public static final String REGISTER_EMAIL_HOST = "register.email.host";
    public static final String REGISTER_EMAIL_PORT = "register.email.port";
    public static final String REGISTER_EMAIL_USERNAME = "register.email.username";
    public static final String REGISTER_EMAIL_PASSWORD = "register.email.password";
    public static final String REGISTER_EMAIL_DEFAULT_ENCODING = "register.email.default-encoding";

    private SystemConfig() {
    }
}
